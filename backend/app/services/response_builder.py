...
NO_FILTERS_NOTE = (
    "No filters were applied. The answer covers every row of the dataset."
)
    ...
    if state["status"] == "success" and definition is not None:
        return QueryResponse(
            **common,
            ...
            assumptions=[
                f"This definition comes from the {definition.source} source. "
                "No calculation was performed."
            ],
        )

    # Numerical/tool-based success response
    if state["status"] == "success":
        result = state["validated_result"]

        notes = (
            result.notes
            if isinstance(result, _TOOL_NOTES)
            else []
        )
        ...
        if uses_money(plan):
            assumptions.append(
                f"Amounts are shown with the symbol '{currency_symbol}'. "
                "The data file does not contain a currency."
            )
