...
## Module 7 — Basic RAG with pgvector

- New table `knowledge_documents` (slug unique key, `embedding vector(768)`, `doc_metadata` JSONB).
- 15 seed business-metric definitions, idempotently seeded via
  `python -m app.scripts.seed_knowledge_base` (not run automatically at startup).
- `GeminiEmbeddingService` implements `langchain_core.embeddings.Embeddings`, using the
  same `google-genai` SDK client style as Module 4's `gemini_client.py`. Model:
  `gemini-embedding-001`, 768 dimensions, manually normalized.
- `KnowledgeSearchService`: raw SQL (`text()`), pgvector `<=>` cosine distance, top-3,
  configurable threshold (`KNOWLEDGE_SIMILARITY_THRESHOLD`, default 0.6).
- `KnowledgeBaseMetricRetriever` implements the existing (Module 5) `MetricRetriever`
  Protocol; selected only when `STORAGE_BACKEND=postgres` and a Gemini key is set.
  Falls back to the existing stub response on any failure or no match — see D-064.
- New endpoint: `POST /knowledge/search`.
- `QueryResponse.definition_source` (new, optional field) surfaces in the frontend as
  a badge; `null` unless a real knowledge-base definition was used.
- Requires PostgreSQL — `STORAGE_BACKEND=json` uses the Module 5 stub, unchanged.

### Known limitations

- No ANN index; fine at 15 rows, revisit if the knowledge base grows substantially.
